...
export default function LoginPage() {
  const { session, loading, login, loginGoogle, sendPasswordReset } = useAuth();
  ...
  async function handleForgot(e: React.FormEvent) {
    ...
    if (!result.ok) {
      setForgotMsg({ ok: false, text: result.error ?? "Error al enviar el correo." });
    } else {
      setForgotMsg({ ok: true, text: "Si ese email está registrado, recibirás un correo para restablecer tu contraseña." });
    }
  }
      ...
      <div className="w-full max-w-[390px]">
        ...
        {forgotMode ? (
          ...
          >
            ...
            {forgotMsg && (
              <p className={`rounded-lg px-3 py-2 text-sm ${forgotMsg.ok ? "bg-green-wash text-green-deep" : "bg-red-wash text-red-deep"}`}>
                {forgotMsg.text}
              </p>
            )}
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              ...
              <button
                ...
                className="self-end text-[12.5px] font-semibold text-green"
              >
