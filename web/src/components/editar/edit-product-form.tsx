...
export function EditProductForm() {
  ...
  const { products, updateProduct, isLoaded } = useInventory();

  const [initialized, setInitialized] = useState(false);
  const [name, setName] = useState("");
  const [category, setCategory] = useState<CategoryKey>("lacteos");
  const [state, setState] = useState<ProductState>("abierto");
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-bg">
      ...
      {/* Bottom panel: Botones de acción */}
